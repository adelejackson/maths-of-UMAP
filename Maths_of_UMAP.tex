...
We will now define an adjunction between fuzzy simplicial sets and extended
...
language.
We will define a simplicial complex, then a Delta complex, and then give an
informal definition of a simplicial set.
...
\begin{defn}[Delta complex II]
  Let $\hat{\Delta}$ be the category whose objects are finite ordered sets
  $$[n] := [0,1,\dots,n]$$
  and whose morphisms are strictly order-preserving maps $[m]\to[n]$.
...
\begin{defn}
  ...
  A\to [0, 1]$, where if $\mu(a) = 1$, $a$ is definitely in the fuzzy set.
\end{defn}
